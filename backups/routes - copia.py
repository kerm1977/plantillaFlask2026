...
def get_events():
    ...
    output = []
    for e in events:
        # Separación Inteligente de Seguridad: Día vs Logística
        lugar = e.lugar_salida or ''
        logistica_segura = False
        
        # Detectamos el sello invisible en la base de datos
        if lugar.startswith('SEGURO_'):
            logistica_segura = True
            lugar = lugar.replace('SEGURO_', '')
            
        destino_text = "📍 Logística Privada" if logistica_segura else lugar
        ...
        output.append({
            ...
            "solo_chat": e.solo_chat, # Ahora aplica SOLO a la fecha
...
def toggle_espacio(event_id):
    ...
    if evento.capacidad and str(evento.capacidad).startswith('AGOTADO_'):
        # Restaura la capacidad original quitando la bandera
        evento.capacidad = str(evento.capacidad).replace('AGOTADO_', '')
    elif evento.capacidad == 'AGOTADO':
        evento.capacidad = '14-17' # Fallback por seguridad
    else:
        # Añade la bandera para conservar la capacidad real oculta
...
def manifest():
    ...
    # Retornamos application/manifest+json en lugar de json normal para mayor compatibilidad
...
def update_profile():
    ...
    try:
        ...
        if avatar_file and avatar_file.filename != '':
            ...
            # CORRECCIÓN: Asegurar que se guarde en 'static/uploads'
            static_folder = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'static', 'uploads')
            os.makedirs(static_folder, exist_ok=True)
            
            filepath = os.path.join(static_folder, filename)
            avatar_file.save(filepath)
            # Guardamos la ruta relativa para que Flask la encuentre fácilmente
